...
export function LoginForm({ onRegisterClick, onSuccess, compact }: LoginFormProps) {
    ...
    const handleOAuthSignIn = async (provider: OAuthProvider) => {
        ...
        // Retour sur l'accueil localisé : le client Supabase y échange le code
        // PKCE présent dans l'URL et établit la session (detectSessionInUrl).
        const redirectTo = `${window.location.origin}/${locale || "fr"}`
        const { success, error } = await signInWithProvider(provider, redirectTo)

        // En cas de succès le navigateur part vers le provider : on conserve
        // l'état de chargement jusqu'à la redirection.
        if (!success) {
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button
                            ...
                            disabled={oauthPending !== null || isLoadingForm}
